...
$sql = $conexion2->query("SELECT * FROM datosproveedor ORDER BY id_datoProveedor DESC");
$i =0;
while ($row = mysqli_fetch_array($sql)) {
	$i++;
	$ID_usuario = $row['id_datoProveedor'];
	$nombre = $row['datoPersonalProveedor'];
	$apellido = $row['domicilioPersonal'];
	$edad = $row['telefono'];
	?>
     <tr>
     	<td> <?php echo $i; ?></td>
